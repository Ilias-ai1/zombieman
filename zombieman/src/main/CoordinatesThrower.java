...
public class CoordinatesThrower extends Thread {
	...
	public void run() {
		...
		while (true) {
			if (up || down || right || left) {
				if (up)
					newY = Server.spieler[id].y - Konstante.RESIZE;
				else if (down)
					newY = Server.spieler[id].y + Konstante.RESIZE;
				else if (right)
					newX = Server.spieler[id].x + Konstante.RESIZE;
				else if (left)
					newX = Server.spieler[id].x - Konstante.RESIZE;
	...
	// Findet heraus, auf welchen Karten-Sprites sich der Spieler befindet, und
	// prüft, ob sie gültig sind
	boolean coordinateIsValid(int newX, int newY) {
		if (!Server.spieler[id].lebt)
			return false;

		// prüft, ob der Spieler von der Explosion getroffen wurde
		...
		if ((Server.getMap()[l[0]][c[0]].img.equals("floor-1") || Server.getMap()[l[0]][c[0]].img.contains("explosion")) &&
		...
		if ((Server.getMap()[l[0]][c[0]].img.contains("block") || Server.getMap()[l[0]][c[0]].img.contains("wall")) ||
		...
		if (Server.getMap()[l[0]][c[0]].img.contains("bomb-planted")
